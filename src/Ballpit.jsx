...
class W {
  ...
  update(e) {
    ...
    for (let idx = r; idx < t.count; idx++) {
      ...
      // Central gravity (decoupled from mouse position 'i')
      const pullForce = t.gravity * (t.pullForceMultiplier !== undefined ? t.pullForceMultiplier : 3); // Decreased strength
      B.x += (0 - I.x) * pullForce * e.delta;
      B.y += (0 - I.y) * pullForce * e.delta;
      B.z += (0 - I.z) * pullForce * e.delta;

      // Hover animation
      const hoverFreq = t.hoverFreq !== undefined ? t.hoverFreq : 1.0;
      const hoverAmp = t.hoverAmp !== undefined ? t.hoverAmp : 0.5;
      B.x += Math.sin(e.elapsed * hoverFreq + idx) * hoverAmp * e.delta;
      B.y += Math.cos(e.elapsed * hoverFreq * 1.2 + idx) * hoverAmp * e.delta;
      B.z += Math.sin(e.elapsed * hoverFreq * 0.8 + idx) * hoverAmp * e.delta;
...
const Ballpit = ({ className = '', followCursor = true, ...props }) => {
  ...
  useEffect(() => {
    if (spheresInstanceRef.current) {
      spheresInstanceRef.current.updateConfig(props);
    }
  }, [props.gravity, props.friction, props.wallBounce, props.maxVelocity, props.pullForce, props.minSize, props.maxSize, props.size0, props.hoverAmp, props.hoverFreq]);
